    ...
    for knob in nuke_effect_attribs:
        ...
        if knob == "gain":
            if value.startswith("{"): # Multiple channels
                ...
                fusion_effect_attribs["MasterRedGain"] = f"Input {{Value = {split_value[0]}, }}"
                fusion_effect_attribs["MasterGreenGain"] = f"Input {{Value = {split_value[1]}, }}"
                fusion_effect_attribs["MasterBlueGain"] = f"Input {{Value = {split_value[2]}, }}"
            else: #Master channel
        ...
        if knob == "gamma":
            if value.startswith("{"): # Multiple channels
                split_value = value.replace("{", "").replace("}", "").split(" ")
                fusion_effect_attribs["MasterRedGamma"] = f"Input {{Value = {split_value[0]}, }}"
                fusion_effect_attribs["MasterGreenGamma"] = f"Input {{Value = {split_value[1]}, }}"
                fusion_effect_attribs["MasterBlueGamma"] = f"Input {{Value = {split_value[2]}, }}"
            else: #Master channel
                fusion_effect_attribs["MasterRGBGamma"] = f"Input {{Value = {value}, }}"
        ...
        if ".gamma" in knob:
            ...
            if value.startswith("{"): # Multiple channels
                ...
                fusion_effect_attribs[f"{fusion_knob}RGBGamma"] = f"Input {{Value = {value}, }}"

        if knob == "offset":
            if value.startswith("{"): # Multiple channels
                split_value = value.replace("{", "").replace("}", "").split(" ")
                fusion_effect_attribs["MasterRedBrightness"] = f"Input {{Value = {split_value[0]}, }}"
                fusion_effect_attribs["MasterGreenBrightness"] = f"Input {{Value = {split_value[1]}, }}"
                fusion_effect_attribs["MasterBlueBrightness"] = f"Input {{Value = {split_value[2]}, }}"
            else: #Master channel
                fusion_effect_attribs["MasterRGBBrightness"] = f"Input {{Value = {value}, }}"
        ...
        if ".offset" in knob:
            ...
            if value.startswith("{"): # Multiple channels
                ...
                fusion_effect_attribs[f"{fusion_knob}RGBBrightness"] = f"Input {{Value = {value}, }}"
